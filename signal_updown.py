...
def _s_ofi(binance_sym: str) -> float:
    """
    OFI = (buy_vol - sell_vol) / (buy_vol + sell_vol) over last 60 seconds.
    ...
    # Use last 60 seconds to capture more trades (especially for lower-vol coins)
    ...
    # Need at least 5 trades (not a volume threshold) to compute signal
    if count < 5:
        return 0.50
...
def compute_updown_signal(market: dict, yes_midprice: float) -> dict:
    ...
    try:
        ...
        T_remaining = (end_dt - now_utc).total_seconds() / 60.0
    except Exception:
        T_remaining = 2.5

    # Hard guard: skip expired markets immediately
    if T_remaining <= 0:
        return _pass("market expired")
    ...
    return {
        ...
        "crypto_price":   round(current_price, 4),
